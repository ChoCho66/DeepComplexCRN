...
from dataset import THCHS30Dataset
...
# 建立參數解析器
parser = argparse.ArgumentParser(description='Process audio dataset paths')
parser.add_argument('--noisy_files_path', type=str, 
                   default='/disk4/chocho/_datas/THCHS-30-train-noisy/train-noisy',
                   help='Path to noisy audio files')
parser.add_argument('--clean_files_path', type=str, 
                   default='/disk4/chocho/_datas/THCHS-30/data_thchs30/train',
                   help='Path to clean audio files')
...
# 使用解析後的參數建立資料集
train_dataset = THCHS30Dataset(
    noisy_files_path = args.noisy_files_path,
    clean_files_path = args.clean_files_path,
)
...
# 只訓練前面的層，凍結 DCCRN 最後 N 層
N = 3  # 設定要凍結的層數
all_layers = list(model.named_parameters())  # 取得所有層的名稱與參數

for name, param in all_layers[-N:]:  # 只針對最後 N 層
    param.requires_grad = False  # 凍結參數
    print(f"Freezing layer: {name}")  # 確認哪些層被凍結
print('----'*10)
...
torch.save({'model': model.state_dict()}, os.path.join(
    checkpoint_dir, f'dccrn_finetune_on_THCHS30_epoch{epoch+1}.pt'))
